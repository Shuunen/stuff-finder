...
export const button = (content: string, classes = '', secondary = false): HTMLButtonElement => {
  ...
  return dom('button', `text-lg h-10 max-w-xs px-12 md:text-base bg-gradient-to-tr shadow-md hover:shadow-lg rounded transition-all duration-200 each-in-out sm:px-6 ${theme} ${classes}`, content)
}
