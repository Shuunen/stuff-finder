...
export function AppBarcode({ isHighlighted = false, item, size, willResize = true }: Readonly<{ isHighlighted?: boolean; item: Item; size: PrintSize; willResize?: boolean }>) {
  ...
  return (
    <div
      class="box-content flex items-center gap-0 overflow-hidden rounded-sm border border-black px-1 transition-all print:rounded-none print:border-0 print:px-0"
      data-component="barcode"
      ref={wrapperReference}
      style={printSizes[size].styles}
    >
      <div class={`mt-1 ${isHighlighted ? 'bg-green-400' : ''}`}>
        {/* @ts-expect-error missing types */}
        <qr-code data={value} margin={0} modulesize={3} ref={wcReference} />
      </div>
      <div class="overflow-hidden pt-1 pl-1.5 text-center">
        <div class={`mb-1 line-clamp-3 font-sans text-[12px] leading-4 tracking-[-0.5px] ${isHighlighted ? 'bg-red-400' : ''}`}>{printText}</div>
        <div class={`mb-1 font-mono text-[19px] leading-none font-bold tracking-[2px] ${isHighlighted ? 'bg-blue-400' : ''}`}>{printLocation}</div>
      </div>
    </div>
  )
}
