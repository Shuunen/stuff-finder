...
import type { AppLoaderToggleEvent } from '../types/events.types'
...
window.customElements.define('app-loader', class extends HTMLElement {
  public connectedCallback () {
    ...
    on<AppLoaderToggleEvent>('app-loader--toggle', async isActive => { logger.info(`toggle loader ${isActive ? 'on' : 'off'}`); await toggle(isActive) })
