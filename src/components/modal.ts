...
window.customElements.define('app-modal', class extends HTMLElement {
  ...
  createModal (id: string) {
    const modal = div(`${id} w-full max-w-md sm:w-auto sm:max-w-full flex flex-col m-4 p-4 bg-white relative overflow-x-hidden overscroll-y-auto rounded ${this.className}`, this.innerHTML)
    const close = link('close absolute text-4xl md:text-2xl opacity-50 font-mono top-2 right-5', 'x', '#')
    modal.append(close)
    if (this.dataset.title) modal.prepend(h2('header text-purple-700 text-2xl mb-4 leading-7 text-center', this.dataset.title))
  ...
  connectedCallback () {
    ...
    this.backdrop.dataset.action = 'app-modal--close'
