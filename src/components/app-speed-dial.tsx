...
export function AppSpeedDial () {
  ...
  function toggleOpen (reason = 'unknown') { logger.debug('toggle cause', reason); setOpen(!isOpen); return voidFunction }
  function onMouse (status: 'enter' | 'leave') { if (!scout.isMobile) { logger.debug('open cause mouse', status); setOpen(status === 'enter') }; return voidFunction }
